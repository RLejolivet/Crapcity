...
//	private int i_numOfPlayers = 1;
	...
	private string s_ipMasterServer = "ip address of MasterServer";
	...
	private void RefreshHostList(string _s_ipMasterServer)
	{
		MasterServer.port = 23466;
		MasterServer.ipAddress = _s_ipMasterServer;
...
//		}
	...
	private void showMultiplayerMenu()
	{
		...
		{
			GUI.Label(new Rect (25, 190, 700, 220), "Please enter the ip address of the server: ");
			s_ipMasterServer = GUI.TextField(new Rect(25, 220, 500, 30), s_ipMasterServer);

			if(GUI.Button(new Rect(25, 250, 500, 30), "searchServer"))
			{
				if(s_ipMasterServer != "ip address of MasterServer") //A work left is to test if s_ipMasterServer has the formal of ip address
				{
					RefreshHostList(s_ipMasterServer); 
				}
			}
			...
			if(hostList != null)
			{
				for(int i = 0; i < hostList.Length; ++i)
				{
					if(GUI.Button(new Rect(30, 280+i*55, 150, 30), hostList[i].gameName))
					{
